...
variable {E : Type _} {M M₁ M₂ : Matroid E} {I C C' C₁ C₂ X : Set E} {e f : E}

open Set

namespace Matroid

theorem Circuit.dep (hC : M.Circuit C) : ¬M.indep C :=
  hC.1

theorem Circuit.sSubset_indep (hC : M.Circuit C) (hXC : X ⊂ C) : M.indep X :=
  by_contra fun h => hXC.Ne (hXC.Subset.antisymm (hC.2 h hXC.Subset))

theorem circuit_iff : M.Circuit C ↔ ¬M.indep C ∧ ∀ I, ¬M.indep I → I ⊆ C → I = C :=
  ⟨fun h => ⟨h.1, fun I hI h' => h'.antisymm (h.2 hI h')⟩, fun h =>
    ⟨h.1, fun I hI hIC => (h.2 I hI hIC).symm.Subset⟩⟩

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (I «expr ⊂ » C) -/
theorem circuit_iff_forall_sSubset : M.Circuit C ↔ ¬M.indep C ∧ ∀ (I) (_ : I ⊂ C), M.indep I :=
  ⟨fun hC => ⟨hC.dep, fun I hIC => hC.sSubset_indep hIC⟩, fun h =>
    ⟨h.1, fun X hX hXC =>
      hXC.ssubset_or_eq.elim (fun h' => (hX (h.2 _ h')).elim)
        (by
          rintro rfl
          rfl)⟩⟩
...
theorem circuit_iff_mem_minimals : M.Circuit C ↔ C ∈ minimals (· ⊆ ·) { X | ¬M.indep X } :=
  Iff.rfl

theorem Circuit.eq_of_dep_subset (hC : M.Circuit C) (hX : ¬M.indep X) (hXC : X ⊆ C) : X = C :=
...
theorem Circuit.nonempty (hC : M.Circuit C) : C.Nonempty :=
  ...
  rintro rfl
  exact hC.1 M.empty_indep

theorem empty_not_circuit (M : Matroid E) : ¬M.Circuit ∅ := fun h => by simpa using h.nonempty
...
theorem Circuit.eq_of_dep_subset_self (hC : M.Circuit C) (hX : ¬M.indep X) (hXC : X ⊆ C) : C = X :=
  by_contra fun h => hX (hC.sSubset_indep (ssubset_of_subset_of_ne hXC (Ne.symm h)))

theorem Circuit.eq_of_subset_circuit (hC₁ : M.Circuit C₁) (hC₂ : M.Circuit C₂) (h : C₁ ⊆ C₂) :
    C₁ = C₂ :=
  (hC₂.eq_of_dep_subset_self hC₁.dep h).symm

theorem Circuit.circuit_lrestrict_of_subset (hC : M.Circuit C) (hCX : C ⊆ X) :
    (M.lrestrict X).Circuit C :=
  by
  simp_rw [circuit_iff_forall_ssubset, lrestrict_indep_iff, not_and]
  exact ⟨fun h => (hC.dep h).elim, fun I hIC => ⟨hC.ssubset_indep hIC, hIC.subset.trans hCX⟩⟩
...
def fundCircuit (M : Matroid E) (e : E) (I : Set E) :=
...
theorem mem_fundCircuit (M : Matroid E) (e : E) (I : Set E) : e ∈ fundCircuit M e I :=
  mem_insert _ _
...
theorem Indep.fundCircuit_circuit (hI : M.indep I) (he : e ∈ M.cl I \ I) :
  ...
  have hI' : I ∈ { J : Set E | J ⊆ I ∧ e ∈ M.cl J } := ⟨rfl.subset, he.1⟩
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » X) -/
theorem exists_circuit_subset_of_dep (hX : ¬M.indep X) : ∃ (C : _)(_ : C ⊆ X), M.Circuit C :=
  by
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » X) -/
theorem dep_iff_supset_circuit : ¬M.indep X ↔ ∃ (C : _)(_ : C ⊆ X), M.Circuit C :=
  ⟨exists_circuit_subset_of_dep, fun ⟨C, hCX, hC⟩ hX => hC.dep (hX.Subset hCX)⟩

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » I) -/
theorem indep_iff_forall_subset_not_circuit : M.indep I ↔ ∀ (C) (_ : C ⊆ I), ¬M.Circuit C :=
  by
  rw [← not_iff_not]
  simp_rw [dep_iff_supset_circuit, not_forall, Classical.not_not]

theorem Circuit.subset_cl_diff_singleton (hC : M.Circuit C) (e : E) : C ⊆ M.cl (C \ {e}) :=
  by
  by_cases he : e ∈ C
  · rw [(hC.diff_singleton_basis he).cl]
    exact subset_cl _ _
...
theorem mem_cl_iff_exists_circuit :
  ...
  · by_contra' h'
    ...
    have heI : e ∈ M.cl I \ I := by
      rw [hI.cl]
      exact ⟨h, not_mem_subset hI.subset h'.1⟩
...
theorem mem_cl_iff_exists_circuit_of_not_mem (he : e ∉ X) :
  ...
  rw [mem_cl_iff_exists_circuit, iff_false_intro he, false_or_iff]
...
theorem Circuit.strong_multi_elimination {ι : Type _} (hC : M.Circuit C) (x : ι → E)
    (Cs : ι → Set E) (hCs : ∀ i, M.Circuit (Cs i)) (h_mem : ∀ i, x i ∈ C ∩ Cs i)
  ...
  · rintro x' ⟨i, rfl⟩
    exact mem_Union_of_mem i (h_mem i).2
  exact disjoint_of_subset_right (subset_insert z _) disjoint_sdiff_left

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » «expr \ »(«expr ∪ »(C₁, C₂), {e})) -/
...
theorem Circuit.strong_elimination (hC₁ : M.Circuit C₁) (hC₂ : M.Circuit C₂) (he : e ∈ C₁ ∩ C₂)
  ...
  obtain ⟨C, hC, hfC, hCss⟩ :=
    @circuit.strong_multi_elimination E M C₁ Unit hC₁ (fun _ => e) (fun _ => C₂) (by simpa)
...
theorem Circuit.elimination (hC₁ : M.Circuit C₁) (hC₂ : M.Circuit C₂) (h : C₁ ≠ C₂) (e : E) :
...
theorem Circuit.eq_fundCircuit_of_subset_insert_indep (hC : M.Circuit C) (hI : M.indep I)
  ...
  have he : e ∉ I := by
    intro heI
    rw [insert_eq_of_mem heI] at hCI
    exact hC.dep (hI.subset hCI)
  have heI : e ∈ M.cl I \ I :=
    by
...
theorem eq_of_circuit_iff_circuit_forall {M₁ M₂ : Matroid E}
    (h : ∀ C, M₁.Circuit C ↔ M₂.Circuit C) : M₁ = M₂ :=
  eq_of_indep_iff_indep_forall fun I => by simp_rw [indep_iff_forall_subset_not_circuit, h]
...
theorem indep_iff_forall_finite_subset_indep [Finitary M] :
  ...
  ⟨fun h J hJI hJ => h.Subset hJI, fun h =>
    indep_iff_forall_subset_not_circuit.mpr fun C hCI hC => hC.dep (h C hCI hC.Finite)⟩
...
@[simp]
theorem dual_circuit_iff_cocircuit {K : Set E} : M﹡.Circuit K ↔ M.Cocircuit K :=
  Iff.rfl

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (K «expr ⊆ » X) -/
theorem coindep_iff_forall_subset_not_cocircuit : M.Coindep X ↔ ∀ (K) (_ : K ⊆ X), ¬M.Cocircuit K :=
  by
  rw [← dual_indep_iff_coindep, indep_iff_forall_subset_not_circuit, forall_congr']
  simp

theorem cocircuit_iff_mem_minimals {K : Set E} :
    M.Cocircuit K ↔ K ∈ minimals (· ⊆ ·) { X | ∀ B, M.base B → (B ∩ X).Nonempty } := by
  simp_rw [cocircuit, circuit, dual_indep_iff_coindep, coindep, not_exists, not_and,
    not_disjoint_iff_nonempty_inter, inter_comm]

theorem Cocircuit.finite [Finitary (M﹡)] {K : Set E} (hK : M.Cocircuit K) : K.Finite :=
...
theorem girth_eq_iff {k : ℕ} (hk : k ≠ 0) :
    M.girth = k ↔
...
theorem Base.strong_exchange (hB₁ : M.base B₁) (hB₂ : M.base B₂) (he : e ∈ B₁ \ B₂) :
  ...
  by_contra' h'
  have hecl : e ∈ M.cl B₂ \ B₂ :=
    ⟨by
      rw [hB₂.cl]
      exact mem_univ _, he.2⟩
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » I) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » I) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (Cf «expr ⊆ » insert e K) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » «expr \ »(«expr ∪ »(C₁, C₂), {e})) -/
/-- A collection of sets satisfying the circuit axioms determines a matroid -/
def matroidOfCircuitOfFinite [Finite E] (circuit : Set E → Prop) (empty_not_circuit : ¬circuit ∅)
    (antichain : ∀ C₁ C₂, circuit C₁ → circuit C₂ → C₁ ⊆ C₂ → C₁ = C₂)
    (elimination :
      ∀ C₁ C₂ e,
        circuit C₁ →
          circuit C₂ → C₁ ≠ C₂ → e ∈ C₁ ∩ C₂ → ∃ (C : _)(_ : C ⊆ (C₁ ∪ C₂) \ {e}), circuit C) :
    Matroid E :=
  matroidOfIndepOfFinite (fun I => ∀ (C) (_ : C ⊆ I), ¬circuit C)
    ⟨∅, fun C hC => by rwa [subset_empty_iff.mp hC]⟩ (fun I J hIJ hJ C hCI => hIJ C (hCI.trans hJ))
    (by
      by_contra' h
      obtain ⟨I, J, hI, hJ, hIJ, Hbad⟩ := h
      set indep := fun I => ∀ (C) (_ : C ⊆ I), ¬circuit C with hi
      ...
      obtain ⟨K, ⟨hK, hKIJ, hIK⟩, hKmin⟩ :=
        @Set.Finite.exists_minimal_wrt (Set E) _ _ (fun X => (I \ X).ncard) sbad (to_finite sbad)
          hbad_ne
      ...
      · obtain ⟨e, heK, heI⟩ := exists_mem_not_mem_of_ncard_lt_ncard hIK
        have heJ : e ∈ J := by_contra fun heJ => not_or_of_not heI heJ (hKIJ heK)
        obtain ⟨C, hCeI, hC⟩ := Hbad e heJ heI
        exact hK C (hCeI.trans (insert_subset.mpr ⟨heK, diff_eq_empty.mp hIK_empty⟩)) hC
      have hCf : ∀ f ∈ K \ I, ∃ (Cf : _)(_ : Cf ⊆ insert e K), circuit Cf ∧ f ∉ Cf ∧ e ∈ Cf :=
        by
        rintro f ⟨hfK, hfI⟩
        have hef : e ≠ f := fun h => hfI (h ▸ heI)
        set T := insert e K \ {f} with hT
        have hTIJ : T ⊆ I ∪ J := (diff_subset _ _).trans (insert_subset.mpr ⟨Or.inl heI, hKIJ⟩)
        have hTcard : T.ncard = K.ncard := by rw [hT, ncard_exchange' heK hfK]
        have hITcard : (I \ T).ncard < (I \ K).ncard :=
          by
          rw [Nat.lt_iff_add_one_le, hT, ← insert_diff_singleton_comm hef, ← union_singleton, ←
            diff_diff, ncard_diff_singleton_add_one]
          ...
          exact ⟨heI, fun he' => heK he'.1⟩
        have hTi : ¬indep T := fun hTi =>
          hITcard.ne (hKmin _ hTi hTIJ (hIK.trans_eq hTcard.symm) hITcard.le).symm
        push_neg  at hTi
        obtain ⟨Cf, hCfT, hCf⟩ := hTi
        refine' ⟨Cf, hCfT.trans (diff_subset _ _), hCf, _, _⟩
        · exact mt (@hCfT f) (not_mem_diff_of_mem (mem_singleton f))
        refine' by_contra fun heCf => hK Cf (fun x hxCf => _) hCf
        exact
          mem_of_mem_insert_of_ne (hCfT hxCf).1
            (by
              rintro rfl
              exact heCf hxCf)
      ...
      have hg_ex : ∃ g', g' ∈ Cg ∧ g' ∈ K \ I :=
        ...
        exact
          hI _
            (fun x hx =>
              Or.elim (hCgss hx) (fun h => h.symm ▸ heI) fun hxK =>
                by_contra fun hxI => hg' _ hx ⟨hxK, hxI⟩)
            hCg
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » «expr \ »(«expr ∪ »(C₁, C₂), {e})) -/
@[simp]
theorem matroidOfCircuitOfFinite_apply [Finite E] (circuit : Set E → Prop)
    ...
    (elimination :
      ∀ C₁ C₂ e,
        circuit C₁ →
          circuit C₂ → C₁ ≠ C₂ → e ∈ C₁ ∩ C₂ → ∃ (C : _)(_ : C ⊆ (C₁ ∪ C₂) \ {e}), circuit C) :
    (matroidOfCircuitOfFinite circuit empty_not_circuit antichain elimination).Circuit = circuit :=
  by
  ext C
  simp_rw [matroid_of_circuit_of_finite, Matroid.circuit_iff_forall_sSubset,
    matroid_of_indep_of_finite_apply, not_forall, Classical.not_not, exists_prop]
  refine'
    ⟨fun h => _, fun h =>
      ⟨⟨_, rfl.subset, h⟩, fun I hIC C' hC'I hC' =>
