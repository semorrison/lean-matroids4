...
! This file was ported from Lean 3 source module mathlib.ncard
-/
...
theorem nonempty_of_ncard_ne_zero (hs : s.ncard ≠ 0) : s.Nonempty :=
  by
...
theorem ncard_singleton_inter : ({a} ∩ s).ncard ≤ 1 :=
  by
...
theorem ncard_le_ncard_insert (a : α) (s : Set α) : s.ncard ≤ (insert a s).ncard :=
  by
  refine'
    s.finite_or_infinite.elim (fun h => _) fun h =>
      by
      rw [h.ncard]
      exact Nat.zero_le _
...
@[simp]
theorem card_doubleton (h : a ≠ b) : ({a, b} : Set α).ncard = 2 :=
  by
...
theorem exists_eq_insert_iff_ncard
    (hs : s.Finite := by
      run_tac
        to_finite_tac) :
    (∃ (a : _)(_ : a ∉ s), insert a s = t) ↔ s ⊆ t ∧ s.ncard + 1 = t.ncard :=
  by
  constructor
  · rintro ⟨a, ha, rfl⟩
...
theorem ncard_le_one_of_subsingleton [Subsingleton α] (s : Set α) : s.ncard ≤ 1 :=
  by
...
theorem ncard_eq_two : s.ncard = 2 ↔ ∃ x y, x ≠ y ∧ s = {x, y} :=
  by
  refine' ⟨fun h => _, _⟩
